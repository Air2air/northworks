...
export default function Breadcrumbs({ items, maxWidth = '200px' }: BreadcrumbsProps) {
  return (
    <nav className="flex px-4 py-3 text-blue-700 bg-blue-50" aria-label="Breadcrumb">
        ...
        {items.map((item, index) => {
          ...
          return (
            <li key={item.href} className={`flex items-center ${isLast ? 'flex-1 min-w-0' : 'flex-shrink-0'}`}>
              {index > 0 && (
                <svg
                  className="w-6 h-6 text-blue-200 flex-shrink-0"
                  fill="currentColor"
                  viewBox="0 0 20 20"
              ...
              {item.active ? (
                <span 
                  className={`ml-1 md:ml-2 text-sm font-medium text-blue-900 ${isLast ? 'truncate min-w-0' : ''}`}
                ...
                <Link
                  href={item.href}
                  className="ml-1 md:ml-2 text-sm font-medium text-blue-600 hover:text-blue-800 flex-shrink-0"
                >
                  {item.label}
                </Link>
              )}
