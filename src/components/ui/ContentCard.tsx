...
export function ContentCard({
  ...
  const cardClasses = `bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow cursor-pointer min-h-48 flex flex-col ${className}`;
  ...
  const CardContent = () => (
    <div className="flex flex-1">
      {/* Image */}
      <div className="flex-shrink-0 w-56 mr-4 self-stretch relative">
        {showImage && primaryImage ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-sky-100 to-sky-300 rounded-lg flex items-center justify-center">
      ...
      <div className="flex-1 p-6">
        ...
        {showTags && item.tags && item.tags.length > 0 && (
          <Tags tags={item.tags} variant="medium" />
        )}
      </div>
    </div>
  );
