...
import type { BreadcrumbItem, ContentDetailLayoutProps } from '@/types/index';
            ...
            <UnifiedMetadata
              fields={(() => {
                ...
                if (frontmatter.publication.date) {
                  fields.push({
                    label: 'Date',
                    value: formatDate(frontmatter.publication.date)
                  });
                }
                if (frontmatter.publication.outlet || frontmatter.publication.publisher) {
                  fields.push({
                    label: 'Publication',
                    value: frontmatter.publication.outlet || frontmatter.publication.publisher
                  });
                }
                if (frontmatter.publication.section) {
                  fields.push({
                    label: 'Section',
                    value: frontmatter.publication.section
                  });
                }
                if (frontmatter.publication.author) {
                  fields.push({
                    label: 'Author',
                    value: frontmatter.publication.author
                  });
                }
                return fields;
              })()}
